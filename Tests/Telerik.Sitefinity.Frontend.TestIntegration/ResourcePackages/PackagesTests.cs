...
namespace Telerik.Sitefinity.Frontend.TestIntegration.ResourcePackages
{
    ...
    public class PackagesTests
    {
        ...
        public void ResourcePackage_RenamePackageWithLayoutFiles_VerifyGeneratedTemplates()
        {
            ...
            try
            {
                FeatherServerOperations.ResourcePackages().AddNewResourcePackage(packageResource2);
                FeatherServerOperations.ResourcePackages().WaitForTemplatesCountToIncrease(templatesCount, 1);

                var template = this.PageManager.GetTemplates().Where(t => t.Title == templateLayoutName).FirstOrDefault();
                Assert.IsNotNull(template, "Template with name " + templateLayoutName + " was not found");

                FeatherServerOperations.ResourcePackages().RenamePackageFolder(packageName, newPackageName);
                FeatherServerOperations.ResourcePackages().WaitForTemplatesCountToIncrease(templatesCount, 2);

                var templateRenamed = this.PageManager.GetTemplates().Where(t => t.Title == newTemplateLayoutName).FirstOrDefault();
                Assert.IsNotNull(templateRenamed, "Template with name " + newTemplateLayoutName + " was not found");
            }
            finally
            {
                ServerOperations.Templates().DeletePageTemplate(templateLayoutName);
                ServerOperations.Templates().DeletePageTemplate(newTemplateLayoutName);

                string path = FeatherServerOperations.ResourcePackages().GetResourcePackagesDestination(newPackageName);
                FeatherServerOperations.ResourcePackages().DeleteDirectory(path);
            }
        }
